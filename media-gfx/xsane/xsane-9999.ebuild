...
SRC_URI="
	https://dev.gentoo.org/~pacho/${PN}/${PN}-256x256.png
"
	EGIT_REPO_URI="https://gitlab.com/sane-project/frontend/xsane"
	inherit git-r3
fi
...
PATCHES=(
	...
	# https://gitlab.com/sane-project/frontend/xsane/-/issues/74
	"${FILESDIR}"/xsane-9999-zlib_ld.patch
)
...
src_install() {
	...
	if use gimp; then
		local plugindir gimptool=( "${ESYSROOT}"/usr/bin/gimptool-3 )  # https://bugs.gentoo.org/965456
		if [[ ${#gimptool[@]} -gt 0 ]]; then
			plugindir="$("${gimptool[0]}" --gimpplugindir)/plug-ins"
		else
			die "Can't find GIMP plugin directory."
		fi
		dosym -r /usr/bin/xsane "${plugindir}"/xsane/xsane
	fi

	newicon "${DISTDIR}/${PN}-256x256.png" "${PN}".png

#https://gitlab.com/sane-project/frontend/xsane/-/issues/42
#	dodir /usr/share/sane/xsane
#	insinto /usr/share/sane/xsane
#	doins "${S}"/src/xsane-startimage.pnm
}
